...
public class WireV1 implements Comparable<WireV1> {
	...
	public WireV1(World world, BlockPos pos, BlockState state) {
	...
	@Override
	public int compareTo(WireV1 wire) {
	...
	public void updateConnections() {
		...
		List<WireConnectionV1> oldConnections = new ArrayList<>(connections);
		connections.clear();
	...
	private void addConnection(BlockPos pos, BlockState state, boolean out, boolean in) {
		WireV1 wire = ((IWireBlock)wireBlock).getWireV1(world, pos, state, true, false);
		WireConnectionV1 connection = new WireConnectionV1(wire, out, in);
		
		connections.add(connection);
	}
	
	private void connectionsChanged(List<WireConnectionV1> oldConnections) {
		ignoreUpdates = true;
		
		for (WireConnectionV1 connection : connections) {
			boolean changed = true;
			
			Iterator<WireConnectionV1> it = oldConnections.iterator();
			while (it.hasNext()) {
				WireConnectionV1 oldConnection = it.next();
		...
		for (WireConnectionV1 oldConnection : oldConnections) {
