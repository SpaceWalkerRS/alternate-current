...
public class WireHandler {
	...
	private Node getNextNode(BlockPos pos, BlockState state) {
		return state.is(Block.field_408) ? new WireNode(world, pos, state) : getNextNode().set(pos, state, true);
	}
	...
	private Node revalidateNode(Node node) {
		...
		boolean isWire = state.is(Block.field_408);
	...
	private void discover(WireNode wire) {
		...
		if (!wire.removed && !wire.shouldBreak && !wire.state.canSurviveAt(world, wire.pos)) {
	...
	private void findExternalPower(WireNode wire) {
		...
		wire.externalPower = hasExternalPower(wire) ? POWER_MAX : POWER_MIN;
...
//			profiler.end();
	...
	private void updateBlock(Node node, BlockPos neighborPos, Block neighborBlock) {
		...
		if (!state.isAir() && !state.is(Block.field_408)) {
