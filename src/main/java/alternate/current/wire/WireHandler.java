...
public class WireHandler {
	...
	private void discover(WireNode wire) {
		...
		if (!wire.removed && !wire.shouldBreak && !wire.state.method_16877(world, wire.pos)) {
	...
	private int getExternalPower(WireNode wire) {
		...
		for (int iDir = 0; iDir < Directions.ALL.length; iDir++) {
			...
			if (neighbor.isSignalSource()) {
				power = Math.max(power, neighbor.state.method_16864(world, neighbor.pos, Directions.ALL[iDir]));
			}
	...
	private int getDirectSignalTo(WireNode wire, Node node, int except) {
		...
		for (int iDir : Directions.I_EXCEPT[except]) {
			...
			if (neighbor.isSignalSource()) {
				power = Math.max(power, neighbor.state.method_16886(world, neighbor.pos, Directions.ALL[iDir]));
...
//			profiler.end();
	...
	private void updateShape(Node node, Direction dir, BlockPos neighborPos, BlockState neighborState) {
		...
		if (!state.isAir() && state.getBlock() != Blocks.REDSTONE_WIRE) {
			BlockState newState = state.method_16883(dir, neighborState, world, pos, neighborPos);
	...
	private void updateBlock(Node node, BlockPos neighborPos, Block neighborBlock) {
		...
		if (!state.isAir() && state.getBlock() != Blocks.REDSTONE_WIRE) {
			state.method_16872(world, pos, neighborBlock, neighborPos);
		}
	}
