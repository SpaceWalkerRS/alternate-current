...
public class WireHandler {
	...
	private int getExternalPower(WireNode wire) {
		...
		for (int iDir = 0; iDir < Directions.ALL.length; iDir++) {
			...
			if (neighbor.isSignalSource()) {
				power = Math.max(power, neighbor.state.getBlock().getWeakRedstonePower(world, neighbor.pos, neighbor.state, Directions.ALL[iDir]));
			}
	...
	private int getDirectSignalTo(WireNode wire, Node node, int except) {
		...
		for (int iDir : Directions.I_EXCEPT[except]) {
			...
			if (neighbor.isSignalSource()) {
				power = Math.max(power, neighbor.state.getBlock().getStrongRedstonePower(world, neighbor.pos, neighbor.state, Directions.ALL[iDir]));
...
//			profiler.end();
	...
	private void updateBlock(Node node, BlockPos neighborPos, Block neighborBlock) {
		...
		if (block != Blocks.AIR && block != Blocks.REDSTONE_WIRE) {
			block.neighborUpdate(world, pos, state, neighborBlock);
		}
	}
