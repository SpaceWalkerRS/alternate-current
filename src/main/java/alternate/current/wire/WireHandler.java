...
public class WireHandler {
	...
	public WireHandler(ServerWorld world) {
		this.level = new LevelAccess(world);
	...
	private Node getNextNode(BlockPos pos) {
		...
		if (state.isWire()) {
			return new WireNode((WireBlock)state.getBlock(), level, pos, state);
		}
	...
	/**
	 * Look for a wire at the given position that is in an invalid state and requires 
	 * power changes. This wire is called a 'root' because it is only when this wire
	 * changes power level that neighboring wires must adjust as well.
	 */
	private void findRoot(BlockPos pos, WireType type) {
		Node node = getOrAddNode(pos);
