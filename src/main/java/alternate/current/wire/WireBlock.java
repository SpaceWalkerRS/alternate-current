...
public interface WireBlock {
	...
	default void onWireRemoved(World world, int x, int y, int z, Block block, int metadata) {
		if (!world.isClient) {
			((IServerWorld)world).getWireHandler().onWireRemoved(new BlockPos(x, y, z), new BlockState(block, metadata), getWireType());
		}
	}
