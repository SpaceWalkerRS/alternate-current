...
public class AlternateCurrentCommand {
	...
	private static final DynamicCommandExceptionType NO_SUCH_UPDATE_ORDER = new DynamicCommandExceptionType(value -> new TextComponent("no such update order: " + value));
	...
	private static int queryUpdateOrder(CommandSourceStack source) {
		...
		source.sendSuccess(new TextComponent(String.format("Update order is currently %s", value)), false);
	...
	private static int setUpdateOrder(CommandSourceStack source, UpdateOrder updateOrder) {
		...
		String value = wireHandler.getConfig().getUpdateOrder().id();
		source.sendSuccess(new TextComponent(String.format("update order has been set to %s!", value)), true);
